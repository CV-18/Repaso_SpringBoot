...
public class AlbumRepositoryImpl implements AlbumRepository {
    ...
    public List<Album> findAllByBanda(String banda) {
        ...
        return albumes.values().stream()
                .filter(album -> album.getBanda().toLowerCase().contains(banda.toLowerCase())).toList();
    }

    @Override
    public List<Album> findAllByCategoria(String categoria) {
        log.info("Buscando albumes por su categoria {}",categoria);
        return albumes.values().
                stream()
                .filter(album -> album.getCategoria().toLowerCase().contains(categoria.toLowerCase())).toList();
    }

    @Override
    public List<Album> findByAnio(Integer anio){
        log.info("Buscando albumes por año {}",anio);
        return albumes.values().stream().filter(
                album -> album.getAnio().toString().contains(anio.toString())).toList();
    }
